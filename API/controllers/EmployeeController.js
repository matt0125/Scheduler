...
exports.getEmployeesOfTheSameManager = async (req, res) => {
  try {
    ...
    const teammates = await Employee.find({managedBy: employee.managedBy, _id: { $ne: employeeId}}).select('-_id firstName lastName email phone');

    const manager = await Employee.findById(employee.managedBy).select('-_id firstName lastName email phone');
